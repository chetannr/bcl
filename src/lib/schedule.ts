/**
 * Match schedule data
 * Static schedule for BCL 2025 tournament
 * Round Robin: 30 group stage matches (15 per group) + 3 playoffs = 33 total
 */

export interface MatchResult {
  team1Score: string;
  team2Score: string;
  winner: string;
  margin: string;
}

export interface Match {
  slot: number;
  time: string;
  team1: string;
  team2: string;
  group: 'A' | 'B';
  matchType?: string;
  result?: MatchResult;
}

export interface DaySchedule {
  date: string;
  day: string;
  timeRange: string;
  matchCount: number;
  matches: Match[];
}

export const SCHEDULE: DaySchedule[] = [
  {
    date: '24 Jan',
    day: 'Sat',
    timeRange: '7:00 AM – 5:30 PM',
    matchCount: 12,
    matches: [
      {
        slot: 1,
        time: '7:00 AM',
        team1: 'Phoenix',
        team2: 'Bulldozers',
        group: 'A',
        result: {
          team1Score: '51/3 (6.0 Ov)',
          team2Score: '28/8 (5.0 Ov)',
          winner: 'Phoenix',
          margin: '23 runs',
        },
      },
      {
        slot: 2,
        time: '7:50 AM',
        team1: 'Monsters',
        team2: 'OG',
        group: 'B',
        result: {
          team1Score: '29/2 (6.0 Ov)',
          team2Score: '32/3 (4.5 Ov)',
          winner: 'OG',
          margin: '5 wickets',
        },
      },
      {
        slot: 3,
        time: '8:40 AM',
        team1: 'USA',
        team2: 'Royal Tiger',
        group: 'A',
        result: {
          team1Score: '37/2 (3.5 Ov)',
          team2Score: '35/5 (6.0 Ov)',
          winner: 'USA',
          margin: '6 wickets',
        },
      },
      {
        slot: 4,
        time: '9:30 AM',
        team1: 'YKR',
        team2: 'Super Kings',
        group: 'B',
        result: {
          team1Score: '30/8 (6.0 Ov)',
          team2Score: '52/3 (6.0 Ov)',
          winner: 'Super Kings',
          margin: '22 runs',
        },
      },
      {
        slot: 5,
        time: '10:20 AM',
        team1: 'Phoenix',
        team2: 'Riders',
        group: 'A',
        result: {
          team1Score: '36/1 (6.0 Ov)',
          team2Score: '38/0 (3.3 Ov)',
          winner: 'Riders',
          margin: '9 wickets',
        },
      },
      {
        slot: 6,
        time: '11:10 AM',
        team1: 'Titans',
        team2: 'Sharks',
        group: 'B',
        result: {
          team1Score: '24/7 (6.0 Ov)',
          team2Score: '25/0 (1.3 Ov)',
          winner: 'Sharks',
          margin: '9 wickets',
        },
      },
      { slot: 0, time: '12:00 PM', team1: 'No Match', team2: 'No Match', group: 'A' },
      {
        slot: 7,
        time: '12:50 PM',
        team1: 'Super Kings',
        team2: 'Sharks',
        group: 'B',
        result: {
          team1Score: '46/3 (6.0 Ov)',
          team2Score: '51/5 (6.0 Ov)',
          winner: 'Sharks',
          margin: '5 runs',
        },
      },
      {
        slot: 8,
        time: '1:40 PM',
        team1: 'Phoenix',
        team2: 'USA',
        group: 'A',
        result: {
          team1Score: '63/2 (6.0 Ov)',
          team2Score: '60/1 (6.0 Ov)',
          winner: 'Phoenix',
          margin: '3 runs',
        },
      },
      {
        slot: 9,
        time: '2:30 PM',
        team1: 'Titans',
        team2: 'OG',
        group: 'B',
        result: {
          team1Score: '48/4 (6.0 Ov)',
          team2Score: '49/0 (4.2 Ov)',
          winner: 'OG',
          margin: '9 wickets',
        },
      },
      {
        slot: 10,
        time: '3:20 PM',
        team1: 'Riders',
        team2: 'Bulldozers',
        group: 'A',
        result: {
          team1Score: '35/4 (5.1 Ov)',
          team2Score: '32/1 (6.0 Ov)',
          winner: 'Riders',
          margin: '5 wickets',
        },
      },
      {
        slot: 11,
        time: '4:10 PM',
        team1: 'Monsters',
        team2: 'YKR',
        group: 'B',
        result: {
          team1Score: '55/4 (6.0 Ov)',
          team2Score: '56/2 (4.5 Ov)',
          winner: 'YKR',
          margin: '6 wickets',
        },
      },
      {
        slot: 12,
        time: '5:00 PM',
        team1: 'Riders',
        team2: 'RCB',
        group: 'A',
        result: {
          team1Score: '28/8 (5.0 Ov)',
          team2Score: '45/6 (6.0 Ov)',
          winner: 'RCB',
          margin: '17 runs',
        },
      },
    ],
    
  },
  {
    date: '25 Jan',
    day: 'Sun',
    timeRange: '7:00 AM – 3:30 PM',
    matchCount: 10,
    matches: [
     { slot: 13, time: '7:00 AM', team1: 'RCB', team2: 'Royal Tiger', group: 'A' ,
     result: {
          team1Score: '59/3 (6.0 Ov)',
          team2Score: '40/1 (6.0 Ov)',
          winner: 'RCB',
          margin: '19 runs',
        },
     },
     { slot: 14, time: '7:50 PM', team1: 'Bulldozers', team2: 'USA', group: 'A', 
     result: {
          team1Score: '44/7 (6.0 Ov)',
          team2Score: '45/2 (5.5 Ov)',
          winner: 'USA',
          margin: '6 wickets',
        },
     },
     { slot: 15, time: '8:40 AM', team1: 'RCB', team2: 'Phoenix', group: 'A',
     result: {
          team1Score: '45/6 (6.0 Ov)',
          team2Score: '55/2 (6.0 Ov)',
          winner: 'Phoenix',
          margin: '10 runs',
        },
     },
     { slot: 16, time: '9:30 AM', team1: 'Monsters', team2: 'Sharks', group: 'B',
     result: {
          team1Score: '62/3 (6.0 Ov)',
          team2Score: '58/2 (6.0 Ov)',
          winner: 'Monsters',
          margin: '5 wickets',
        },
     },
     { slot: 17, time: '10:20 AM', team1: 'Titans', team2: 'Super Kings', group: 'B',
      result: {
          team1Score: '36/7 (6.0 Ov)',
          team2Score: '34/2 (6.0 Ov)',
          winner: 'Titans',
          margin: '2 runs',
        },
     },
     { slot: 18, time: '11:10 AM', team1: 'Royal Tiger', team2: 'Phoenix', group: 'A',
     result: {
          team1Score: '54/7 (6.0 Ov)',
          team2Score: '46/1 (6.0 Ov)',
          winner: 'Phoenix',
          margin: '8 runs',
        },
     },
     { slot: 19, time: '12:00 PM', team1: 'OG', team2: 'YKR', group: 'B',
     result: {
          team1Score: '30/5 (6.0 Ov)',
          team2Score: '33/1 (4.5 Ov)',
          winner: 'OG',
          margin: '7 wickets',
        },
     },
     { slot: 20, time: '12:50 PM', team1: 'Super Kings', team2: 'Monsters', group: 'B',
     result: {
          team1Score: '59/2 (6.0 Ov)',
          team2Score: '42/2 (6.0 Ov)',
          winner: 'Super Kings',
          margin: '17 runs',
        },
     },
     { slot: 21, time: '1:40 PM', team1: 'YKR', team2: 'Sharks', group: 'B',
     result: {
          team2Score: '56/2 (6.0 Ov)',
          team1Score: '40/7 (6.0 Ov)',
          winner: 'Sharks',
          margin: '17 runs',
        },
     },
     { slot: 22, time: '2:30 PM', team1: 'RCB', team2: 'USA', group: 'A',
     
    result: {
          team1Score: '68/1 (6.0 Ov)',
          team2Score: '37/3 (6.0 Ov)',
          winner: 'RCB',
          margin: '31 runs',
        },
     },
    ],
  },
  {
    date: '26 Jan',
    day: 'Mon',
    timeRange: '7:00 AM – 5:00 PM',
    matchCount: 11,
    matches: [
      { slot: 23, time: '7:00 AM', team1: 'OG', team2: 'Super Kings', group: 'B',
      result: {
          team1Score: '42/5 (6.0 Ov)',
          team2Score: '57/7 (6.0 Ov)',
          winner: 'Super Kings',
          margin: '15 runs',
        },
      },
      { slot: 24, time: '7:50 AM', team1: 'RCB', team2: 'Bulldozers', group: 'A',
      
      result: {
          team1Score: '54/7 (6.0 Ov)',
          team2Score: '21/5 (5.2 Ov)',
          winner: 'RCB',
          margin: '33 runs',
        },
      },
      { slot: 25, time: '8:40 AM', team1: 'Titans', team2: 'YKR', group: 'B',
      result: {
          team1Score: '77/5 (6.0 Ov)',
          team2Score: '37/4 (6.0 Ov)',
          winner: 'Titans',
          margin: '41 runs',
        },
      },
      { slot: 26, time: '9:30 AM', team1: 'Royal Tiger', team2: 'Bulldozers', group: 'A',
      result: {
          team1Score: '70/7 (6.0 Ov)',
          team2Score: '64/3 (6.0 Ov)',
          winner: 'Royal Tiger',
          margin: '6 runs',
        },
      },
      { slot: 27, time: '10:20 AM', team1: 'OG', team2: 'Sharks', group: 'B',
      result: {
          team1Score: '46/3 (6.0 Ov)',
          team2Score: '45/1 (6.0Ov)',
          winner: 'OG',
          margin: '5 wickets',
        },
      },
      { slot: 28, time: '11:10 AM', team1: 'Riders', team2: 'Royal Tiger', group: 'A',
      result: {
          team1Score: '24/4 (6.0 Ov)',
          team2Score: '25/1 (2.3 Ov)',
          winner: 'Riders',
          margin: '7 wickets',
        },
      },
      { slot: 29, time: '12:00 PM', team1: 'Monsters', team2: 'Titans', group: 'B', 
      result: {
          team1Score: '35/3 (6.0 Ov)',
          team2Score: '23/7 (4.2 Ov)',
          winner: 'Monsters',
          margin: '12 runs',
        },
      },
      { slot: 30, time: '12:50 PM', team1: 'Riders', team2: 'USA', group: 'A',
      result: {
          team1Score: '46/2 (5.2 Ov)',
          team2Score: '43/3 (6.0Ov)',
          winner: 'Riders',
          margin: '41 runs',
        },
      },
      // Playoff matches
      { 
        slot: 31, 
        time: '2:00 PM', 
        team1: 'RCB', 
        team2: 'Sharks', 
        group: 'A', 
        matchType: 'Semi-Final 1',
        result: {
          team1Score: '45/5 (6.0 Ov)',
          team2Score: '47/2 (5.3 Ov)',
          winner: 'Sharks',
          margin: '6 wickets',
        },
      },
      { 
        slot: 32, 
        time: '2:50 PM', 
        team1: 'OG', 
        team2: 'Riders', 
        group: 'B', 
        matchType: 'Semi-Final 2',
        result: {
          team1Score: '56/2 (6.0 Ov)',
          team2Score: '28/8 (5.4 Ov)',
          winner: 'OG',
          margin: '28 runs',
        },
      },
      { 
        slot: 33, 
        time: '3:50 PM', 
        team1: 'OG', 
        team2: 'Sharks', 
        group: 'A', 
        matchType: 'Final',
        result: {
          team1Score: '39/4 (6.0 Ov)',
          team2Score: '41/1 (3.4 Ov)',
          winner: 'Sharks',
          margin: '7 wickets',
        },
      },
    ],
  },
];
